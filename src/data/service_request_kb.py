...
SERVICE_REQUEST_KB_ENTRY: dict[str, object] = {
    ...
    "content": (
        ...
        "- Bước 3: Quản lý (Manager/Admin) xem xét và đưa ra quyết định: phê duyệt chuyển sang SUBMITTED, từ chối chuyển sang REJECTED (kết thúc).\n"
        ...
        "- Cần Quản lý phê duyệt: Xin laptop mới, xin máy in, xin thiết bị ngoại vi, xin quyền VPN, xin quyền Git repo, xin DB access, xin Microsoft 365 license, xin email alias, cập nhật tên hiển thị/email (xác thực hồ sơ HR), xin truy cập mạng nội bộ, mượn thiết bị tạm thời.\n"
        ...
        "- PENDING_APPROVAL: Chờ Quản lý phê duyệt. Kỹ thuật viên chưa thấy yêu cầu này trong hàng đợi.\n"
        ...
        "- REJECTED: Yêu cầu bị Quản lý từ chối (kèm lý do từ chối cụ thể).\n\n"
        "8. Khi yêu cầu bị từ chối:\n"
        "Nếu bị Quản lý từ chối, trạng thái chuyển sang REJECTED và không chuyển cho kỹ thuật viên. Nhân viên có thể xem lý do từ chối (rejection_reason) trên màn hình chi tiết yêu cầu để trao đổi lại với Quản lý hoặc tạo yêu cầu mới.\n\n"
        ...
        "- Hardware: Xin laptop mới (cần duyệt Manager, SLA 24h, nhóm Workplace IT).\n"
        "- Access: Xin quyền VPN (cần duyệt Manager, SLA 4h, nhóm Network & Security).\n"
        "- Software: Xin Microsoft 365 license (cần duyệt Manager, SLA 8h, nhóm Cloud Productivity).\n"
        ...
        "- Onboarding: Đăng ký mượn thiết bị tạm thời (cần duyệt Manager, SLA 8h, nhóm Workplace IT)."
    ),
    "solution": (
        "Quy trình xử lý Service Request chuẩn:\n"
        "1. Nhân viên gửi yêu cầu từ Service Catalog.\n"
        "2. Nếu cần duyệt: PENDING_APPROVAL -> Quản lý duyệt -> SUBMITTED (hoặc REJECTED nếu từ chối).\n"
    ...
    "runbook": '{"steps": ["User selects service from catalog and submits form", "System checks approval policy: PENDING_APPROVAL or SUBMITTED", "Manager approves or rejects (REJECTED)", "Technician with matching fulfillment group takes over (ASSIGNED)", "Technician starts work (IN_PROGRESS) or requests info (WAITING_FOR_USER)", "Technician fulfills service (FULFILLED)"]}',
